<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=28&duration=3000&pause=1000&color=58A6FF&center=true&vCenter=true&width=700&lines=Hey%2C+I'm+Eduardo+%F0%9F%91%8B;Backend+Engineer+%7C+Architecture+Focused;Domain-Driven+Design+%7C+Clean+Architecture;Building+systems+with+intention." alt="Typing SVG" />

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/eduardohnascimento/)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:duufhvo@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-161B22?style=flat-square&logo=github&logoColor=white)](https://github.com/Eduhn26)
![Profile Views](https://komarev.com/ghpvc/?username=Eduhn26&style=flat-square&color=58A6FF)

</div>

---

### 👨‍💻 About Me

Backend engineer from Campinas, Brazil — focused on **architecture that scales and code that communicates**.

I design systems that are:

- Readable six months later  
- Testable without a running server  
- Explicit about state transitions  
- Resilient by design — not by luck  

Background in the MERN ecosystem, now building layered, domain-driven backends with strict TypeScript and clear architectural boundaries.

---

### 🧠 Engineering Philosophy

- Domain first, framework second  
- Boundaries before features  
- Business rules must live in one place  
- Errors should be semantic, predictable, and intentional  
- Systems should evolve without breaking core invariants  

---

## 🛠️ Tech Stack

<div align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=ts,js,nodejs,nestjs,express,postgres,mongodb,prisma,docker,git,github,linux&theme=dark" alt="My Skills" />
  </a>
</div>

---

### 🚀 Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🃏 Truco Paulista Backend</h3>
      <p>Authoritative multiplayer backend built phase by phase — from a pure Domain layer to real-time WebSocket gameplay with PostgreSQL persistence.</p>
      <p><strong>Domain → Application → Infrastructure → Transport</strong></p>
      <p>Designed to simulate production-level backend architecture with defensible technical decisions.</p>
      <p>
        <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
        <img src="https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white"/>
        <img src="https://img.shields.io/badge/Socket.io-010101?style=flat-square&logo=socketdotio&logoColor=white"/>
        <img src="https://img.shields.io/badge/Prisma-2D3748?style=flat-square&logo=prisma&logoColor=white"/>
      </p>
      <a href="https://github.com/Eduhn26/Truco-Paulista2026">View Repository →</a>
    </td>
    <td width="50%" valign="top">
      <h3>🚚 Fleet Scheduling System</h3>
      <p>Corporate vehicle scheduling platform implementing real-world business constraints and state management.</p>
      <p><strong>Routes → Middleware → Controllers → Services → Models</strong></p>
      <p>Includes JWT authentication, role-based access control, strict validation boundaries, and semantic error handling.</p>
      <p>
        <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white"/>
        <img src="https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white"/>
        <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white"/>
        <img src="https://img.shields.io/badge/Zod-3E67B1?style=flat-square&logo=zod&logoColor=white"/>
      </p>
      <a href="https://github.com/Eduhn26/Fleet-Vehicle-Scheduling">View Repository →</a>
    </td>
  </tr>
</table>

---

### 📊 Stats

<div align="center">
  <img height="150em" src="https://github-readme-stats.vercel.app/api?username=Eduhn26&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF&icon_color=58A6FF&hide=issues"/>
  <img height="150em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Eduhn26&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=58A6FF"/>
</div>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=Eduhn26&theme=tokyonight&hide_border=true&background=0D1117&ring=58A6FF&fire=58A6FF&currStreakLabel=58A6FF" />
</div>

---

### 🎯 Currently Focused On

- Advanced NestJS patterns  
- Hexagonal & modular architecture  
- Automated testing strategies (unit → integration → e2e)  
- Distributed systems fundamentals  
- Observability and system resilience  

---

<div align="center">
  <i>Building systems with intention — not just writing code.</i>
</div>
